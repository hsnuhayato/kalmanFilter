...
static const char* kalmanfilter_spec[] =
  {
    ...
    "version",           "CHOREONOID_PACKAGE_VERSION",
    "vendor",            "AIST",
    "category",          "example",
...
RTC::ReturnCode_t KalmanFilter::onInitialize()
{
  ...
  */
  ...
  std::cout<<"KF dof robot "<<m_robot->numJoints()<<std::endl;
  ...
  */
  ...
  m_sensorR = AccelSensors[0]->link()->R() * AccelSensors[0]-> R_local();
  //m_sensorR = Eigen::Matrix3d::Identity();
...
RTC::ReturnCode_t KalmanFilter::onExecute(RTC::UniqueId ec_id)
{
  ...
  if (m_rpyIn.isNew() ) {
    ...
    m_rpy.data.r = m_rate.data[0];
    m_rpy.data.p = m_rate.data[1];
    m_rpy.data.y = m_rate.data[2];
  ...
  if (m_accIn.isNew()){
    ...
    Eigen::Vector3d acc = m_sensorR * Vector3(m_acc.data.ax, m_acc.data.ay, m_acc.data.az); // transform to imaginary acc data
    //Eigen::Vector3d gyro = m_sensorR *Vector3(m_rate.data.avx, m_rate.data.avy, m_rate.data.avz); // transform to imaginary rate data
    Eigen::Vector3d gyro = m_sensorR *Vector3(m_rate.data[0], m_rate.data[1], m_rate.data[2]);
    ...
    */
    ...
    double sx = m_acc.data[0] - sx_ref, sy = m_acc.data[1] - sy_ref, sz = m_acc.data[2] - sz_ref;


      // hrp::RateGyroSensor* sensor = m_robot->sensor<hrp::RateGyroSensor>("gyrometer");
      Vector3 s = m_sensorR * Vector3(sx, sy, sz);
      ...
      */
      ...
      //Vector3 av = m_sensorR * Vector3(m_rate.data.avx, m_rate.data.avy, m_rate.data.avz);
      //m_rate.data.avx = av(0); m_rate.data.avy = av(1); m_rate.data.avz = av(2); // transform to imaginary rate data

      Vector3 av = m_sensorR * Vector3(m_rate.data[0], m_rate.data[1], m_rate.data[2]);
      m_rate.data[0] = av(0); m_rate.data[1] = av(1); m_rate.data[2] = av(2); // transform to imaginary rate data
      ...
      r_filter.update(m_rate.data[0], m_rpyRaw.data.r);
      p_filter.update(m_rate.data[1], m_rpyRaw.data.p);
      y_filter.update(m_rate.data[2], m_rpyRaw.data.y);
      ...
      Matrix3 imaginaryRotationMatrix = q.toRotationMatrix();
      Matrix3 realRotationMatrix = imaginaryRotationMatrix * m_sensorR; // inverse transform to real data
      Vector3 euler = realRotationMatrix.eulerAngles(2,1,0);
